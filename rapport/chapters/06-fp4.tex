...
\subsubsection{ET5 --- Découpage en frames de 256 échantillons + recouvrement 50\,\%}
\label{sec:fp4-et5}

L'exigence \etref{5} demande que le signal audio soit séparé en frames de 256 échantillons. Notre implémentation produit \textbf{62 frames}\footnote{$\lfloor (8000 - 256) / 128 \rfloor + 1 = 61$ frames complètes ; la 62\textsuperscript{ème} est paddée avec 64 zéros pour aligner sur une grille exacte de hop = 128.} de \textbf{256 échantillons} chacune, séparées par un hop de \textbf{128 échantillons} (recouvrement de 50\,\%).

\paragraph{Preuve visuelle dans Audacity.}
Un fichier de labels Audacity (généré par \verb|scripts/audacity_et5.py|) place une étiquette par frame sur la waveform du WAV produit par notre firmware. Audacity dispose automatiquement les étiquettes qui se chevauchent sur deux rangées distinctes, ce qui rend le recouvrement de 50\,\% directement lisible :

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../assets/FP4/et5_audacity_recouvrement.png}
\caption{Validation visuelle du recouvrement (Audacity, zoom sur les frames F33 à F49 du mot « vrai »). Les labels sur deux rangées prouvent que chaque frame de 32\,ms démarre 16\,ms après la précédente, conformément à la spécification du sujet (hop length = frame size / 2).}
\label{fig:et5-recouvrement}
\end{figure}

\paragraph{Preuve unitaire --- une frame isolée.}
La frame 42 (à $t \approx 672\,\text{ms}$, en plein milieu du mot « vrai ») a été extraite et sauvegardée en tant que fichier \texttt{frame\_42.wav} indépendant. Ouvert dans Audacity, on voit clairement \textbf{les 256 échantillons individuels} qui constituent l'entrée du pipeline MFCC pour une itération :

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../assets/FP4/et5_audacity_frame_unique.png}
\caption{Une frame isolée (32\,ms = 256 samples PCM int16). Chaque point bleu carré est un échantillon, vu par la FFT lors de l'étape 4 du pipeline. La périodicité visible sur ce signal correspond à la fondamentale vocale ($\sim$240\,Hz) et ses premières harmoniques.}
\label{fig:et5-frame-unique}
\end{figure}

\noindent\textbf{ET5 est ainsi validée visuellement et numériquement} : le découpage produit bien 62 frames de 256 échantillons avec recouvrement 50\,\%, et les samples PCM sont préservés sans perte du buffer d'acquisition jusqu'à l'entrée FFT.

\subsubsection{ET6 --- Stratégie : miroir Python bit-exact}
...
\begin{lstlisting}[language=Python, caption={Test bit-exact firmware vs Python}]
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{figures/mfcc_heatmap_vrai_vs_faux.png}
\caption{Matrice MFCC 62×13 pour les mots « vrai » et « faux » prononcés (use case retenu : système de réponse vocale type « Question pour un Champion »). Les motifs visuellement distincts sur les coefficients 0, 3, 4, 6 et 11 confirment que les MFCCs capturent assez d'information discriminante pour la classification CNN à venir (FP6). Différence absolue moyenne sur 806 valeurs : 11 792 (\textasciitilde 36\,\% de la dynamique int16).}
